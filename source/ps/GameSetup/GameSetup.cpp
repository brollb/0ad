...
{
	g_Game = new CGame(config.nonVisual, config.saveReplay);
	...
	if (config.isNetworkHost())
	{
		...
		g_NetClient->SetUserName(config.username);
	...
	else if (config.isNetworkClient())
	{
		...
		bool ok = g_NetClient->SetupConnection(config.hostAddress, PS_DEFAULT_PORT, nullptr);
		ENSURE(ok);
	}
	else
	{
