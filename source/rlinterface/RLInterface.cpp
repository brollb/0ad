...
std::vector<std::string> RLInterface::GetTemplates(const std::vector<std::string> names)
{
    std::lock_guard<std::mutex> lock(m_lock);
...
void RLInterface::EnableHTTP(std::string server_address)
{
    ...
    const char *options[] = {
        "listening_ports", server_address.c_str(),
